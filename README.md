...
Implementation of the Chemical Reaction Optimization (CRO) algorithm for supply chain transportation scheduling.

Classification of transportation nodes into three categories
...
Use of a collaborative transportation scheduling strategy for efficient logistics planning.

Integration of four fundamental CRO reaction operators with an additional repair operator to maintain feasible and cost-effective routing solutions.

Random matrix generation for TPL vehicle scheduling to simulate dynamic enterprise assignments.

Capability to minimize total transportation cost, improve vehicle utilization, and reduce computation time.

Performance comparison with existing Ant Colony Optimization (ACO) and modified ACO-NSO algorithms, demonstrating superior efficiency and robustness.
